...
import { listEvents } from "@/server/services/adminService";
...
export async function GET(req: Request) {
  try {
    ...
    const data = await listEvents({ status, hostUserId, page, limit });
    return ok(data);
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Unknown error";
    const stack = e instanceof Error ? e.stack : undefined;
    console.error("Admin events error:", msg);
    ...
    return serverError(`Failed to load events: ${msg}`);
  }
}
